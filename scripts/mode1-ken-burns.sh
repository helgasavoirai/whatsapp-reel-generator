...
#
# NOTE ON THE .txt INPUTS: each must already contain the WRAPPED text (a
# literal newline inserted at the best word-break if the line would run
# past ~24 chars for the name, ~18 for price, ~34 for the tagline — see
# the wrapText() helper in server.js). FFmpeg's drawtext has no built-in
# auto-wrap, and x=(w-text_w)/2 centers EACH line independently when the
# textfile contains a newline, so pre-wrapped text stays centered exactly
# like single-line text — this is how long product names/taglines avoid
# running off the left/right edges of the frame.
#
...
PHOTO="$1"
MUSIC="$2"
PRODUCT_NAME_FILE="$3"   # pre-wrapped plain text file, NOT the raw string —
                          # avoids all shell/ffmpeg escaping issues with
                          # quotes/commas AND handles the line-wrap
PRICE_FILE="$4"
TAGLINE_FILE="$5"
...
# the BOTTOM (regular weight, white). Each line fades in and slides up
# slightly into position, rather than appearing as a hard cut:
#   - product name: appears at t=2s   | fontsize 60
#   - price:        appears at t=5s   | fontsize 80
#   - tagline:       appears at t=8s   | fontsize 46 (bumped from 40 — read
#     too small next to the name/price in testing)
